...
public class TransactionServiceImpl implements TransactionService {
     ...
     */
    public int getMonthlyRewards(Month month, List<Transaction> filteredList) {
     ...
     */
    public int countRewardsPoint(int spend) {
        int totalPoints = 0;

        //2 points/dollar spent over $100
        if (spend > 100) {
            totalPoints = totalPoints + (spend - 100) * 2 + 50;
        }

        //1 point/dollar spent over $50

        if (spend <= 100 && spend > 50) {
            totalPoints = totalPoints + (spend - 50);
        }
